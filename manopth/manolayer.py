...
class ManoLayer(Module):
    ...
    def __init__(self,
        ...
        smpl_data = ready_arguments(self.mano_path)  # 返回的是一个数据结构
        hands_components = smpl_data['hands_components']  # 找到手分量
        ...
        self.register_buffer('th_faces',
                             torch.Tensor(smpl_data['f'].astype(np.int32)).long())  #这些变量会自动放到gpu上 在使用.cuda()函数时
    ...
    def forward(self,
        ...
        th_v_posed = th_v_shaped + torch.matmul(
            self.th_posedirs, th_pose_map.transpose(0, 1)).permute(2, 0, 1)  # 将tensor的维度换位 注意和view 的区别
