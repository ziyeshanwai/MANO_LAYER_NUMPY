...
def ready_arguments(fname_or_dict, posekey4vposed='pose'):
    ...
    if want_shapemodel:
        # print("dd['shapedirs'] shape is {}".format(dd['shapedirs'].r.shape))
        dd['v_shaped'] = dd['shapedirs'].dot(dd['betas']) + dd['v_template']
        v_shaped = dd['v_shaped']
        J_tmpx = np.dot(dd['J_regressor'], v_shaped[:, 0])
        J_tmpy = np.dot(dd['J_regressor'], v_shaped[:, 1])
        J_tmpz = np.dot(dd['J_regressor'], v_shaped[:, 2])
